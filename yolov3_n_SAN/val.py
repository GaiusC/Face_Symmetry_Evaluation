...
Usage:
    $ python path/to/val.py --data coco128.yaml --weights yolov3_n_SAN.pt --img 640
"""
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov3_n_SAN.pt', help='model.pt path(s)')
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        if opt.task == 'speed':  # speed benchmarks
            # python val.py --task speed --data coco.yaml --batch 1 --weights yolov3_n_SAN.pt yolov3_n_SAN-spp.pt...
            opt.conf_thres, opt.iou_thres, opt.save_json = 0.25, 0.45, False
            for opt.weights in weights:
                run(**vars(opt), plots=False)

        elif opt.task == 'study':  # speed vs mAP benchmarks
            # python val.py --task study --data coco.yaml --iou 0.7 --weights yolov3_n_SAN.pt yolov3_n_SAN-spp.pt...
            for opt.weights in weights:
